...
// types as neeeded:
//
//   - metrics.Counter metrics are reported as Prometheus gauges. This is
//     because Prometheus counters must increase monotonically, while
//     metrics.Counter instances can increase or decrease.
//   - metrics.Histogram metrics are reported as Prometheus summaries. This is
//     because metrics.Histogram instances do not store the counts required to
//     create Prometheus histogram buckets.
//   - metrics.Timers values are converted to fractional seconds. This matches
//     the recommended Prometheus base units.
package prometheus
...
func (c *Collector) Collect(ch chan<- prometheus.Metric) {
	c.registry.Each(func(name string, metric any) {
		...
		case metrics.Histogram:
			...
			ch <- prometheus.MustNewConstMetric(desc("min"), prometheus.GaugeValue, float64(ms.Min()))
			ch <- prometheus.MustNewConstMetric(desc("max"), prometheus.GaugeValue, float64(ms.Max()))
			ch <- prometheus.MustNewConstMetric(desc("mean"), prometheus.GaugeValue, float64(ms.Mean()))

		case metrics.Meter:
			ms := m.Snapshot()
			ch <- prometheus.MustNewConstMetric(desc("rate_mean"), prometheus.GaugeValue, ms.RateMean())
			ch <- prometheus.MustNewConstMetric(desc("rate1"), prometheus.GaugeValue, ms.Rate1())
			ch <- prometheus.MustNewConstMetric(desc("rate5"), prometheus.GaugeValue, ms.Rate5())
			ch <- prometheus.MustNewConstMetric(desc("rate15"), prometheus.GaugeValue, ms.Rate15())
			ch <- prometheus.MustNewConstMetric(desc("count"), prometheus.GaugeValue, float64(ms.Count()))
		...
		case metrics.Timer:
			...
			ch <- prometheus.MustNewConstSummary(desc(""), uint64(ms.Count()), toSeconds(ms.Sum()), qs)
			ch <- prometheus.MustNewConstMetric(desc("min"), prometheus.GaugeValue, toSeconds(ms.Min()))
			ch <- prometheus.MustNewConstMetric(desc("max"), prometheus.GaugeValue, toSeconds(ms.Max()))
			ch <- prometheus.MustNewConstMetric(desc("mean"), prometheus.GaugeValue, toSeconds(ms.Mean()))
		}
	})
}

func (c *Collector) descFromName(name string) func(string) *prometheus.Desc {
	...
	return func(suffix string) *prometheus.Desc {
		...
		return prometheus.NewDesc(fqName, "", nil, labels)
	}
}
